...
import com.anaraya.domain.usecase.ManageAuthFamilyUseCase
...
@HiltViewModel
class AuthFamilyViewModel @Inject constructor(
    private val authUseCase: ManageAuthFamilyUseCase,
    ...
    fun validateRayaNationalId(rayaId: String?): Boolean {
        ...
        return try {
            authUseCase.validateSignUpId(rayaId)
            true
        } catch (e: EmptyDataException) {
    ...
    private fun validateOTPData(rayaId: String?, phoneNumber: String?): Boolean {
        _signUpResponse.update {
            AuthUiState(
                error = null
            )
        ...
        return try {
            authUseCase.validateOTPData(rayaId, phoneNumber)
            true
        } catch (e: EmptyDataException) {
            onValidateOTPDataFailure(e.message)
            false
        }
    }

    private fun onValidateOTPDataFailure(error: String?) {
        when (error) {
            context.getString(R.string.raya_id_is_empty) -> {
                _signUpResponse.update {
                    AuthUiState(
    ...
    private fun validateCheckOTPData(rayaId: String?, phoneNumber: String?, otp: String?): Boolean {
        _signUpResponse.update {
            AuthUiState(
                error = null
            )
        ...
        return try {
            authUseCase.validateOTPData(rayaId, phoneNumber, otp)
            true
        } catch (e: EmptyDataException) {
            onValidateCheckOTPDataFailure(e.message)
            false
        }
    }

    private fun onValidateCheckOTPDataFailure(error: String?) {
        when (error) {
            context.getString(R.string.raya_id_is_empty) -> {
                _signUpResponse.update {
                    AuthUiState(
            ...
            context.getString(R.string.otp_is_empty) -> {
                _signUpResponse.update {
                    AuthUiState(
                        isLoading = false,
                        signUpOtpError = true,
    ...
    fun checkAuth(rayaId: String) {
        ...
        viewModelScope.launch {
            try {
                onCheckAuthSuccess(authUseCase.checkRayaId(rayaId))
            } catch (e: NoInternetException) {
    ...
    fun sendVerifyCode(rayaId: String, phoneNumber: String) {
        _signUpResponse.update {
            it.copy(
                isLoading = true,
                isOtpSent = false,
        ...
        viewModelScope.launch {
            try {
                ...
                onSendVerifyCodeFailure(e.message.toString())
            }
        }
    }
    ...
    fun checkVerifyCode(rayaId: String, phoneNumber: String, otp: String) {
        _signUpResponse.update {
            it.copy(
                isLoading = true,
                isOtpCorrect = false,
        ...
        viewModelScope.launch {
            try {
                ...
                onCheckVerifyCodeFailure(e.message.toString())
            }
        }
    }
